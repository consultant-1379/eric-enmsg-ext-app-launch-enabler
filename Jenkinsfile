...
pipeline {
    ...
    stages {
            ...
            steps {
                git branch: 'master',
                    credentialsId: 'enmadm100_private_key',
                     url: '${GERRIT_MIRROR}/OSS/ENM-Parent/SQ-Gate/com.ericsson.oss.containerisation/eric-enmsg-ext-app-launch-enabler'
                sh '''
                    git remote set-url origin --push https://${GERRIT_HTTP_CREDENTIALS_FUser}@${GERRIT_CENTRAL_HTTP_E2E}/OSS/ENM-Parent/SQ-Gate/com.ericsson.oss.containerisation/eric-enmsg-ext-app-launch-enabler
                '''
            }
        }
        stage('Helm Dep Up ') {
            ...
            steps {
                script {
                    withMaven(jdk: env.JDK_HOME, maven: env.MVN_HOME, options: [junitPublisher(healthScaleFactor: 1.0),artifactsPublisher(disabled: true)]) {
                        env.scripts_ear = env.ENM_FOLDER+'-ear/target/'+env.ENM_FOLDER+'-ear-'+env.IMAGE_VERS+'.ear'
                        if (env.MVN_Deploy) {
                            env.MAVEN_COMMAND =  env.MVN_Deploy
                        } else {
                            env.MAVEN_COMMAND = "deploy:deploy-file -DgroupId=com.ericsson.oss.services.eale -DartifactId=external-application-launch-enabler-service -Dversion=${IMAGE_VERS} -Dpackaging=ear -Dfile=${scripts_ear} -DrepositoryId=releases -Durl=https://arm1s11-eiffel004.eiffel.gic.ericsson.se:8443/nexus/content/repositories/releases/"
                        }
                        int buildResult = sh (script: """cd ${ENM_FOLDER};mvn versions:set -DnewVersion=${IMAGE_VERS} -DgenerateBackupPoms=false;mvn ${MVN_PCR};mvn ${MAVEN_COMMAND};cd ..""", returnStatus: true)
                        if (buildResult != 0) {
        ...
        stage('Build Image and Chart') {
            steps {
                sh "${bob} build-helm build-image-with-all-tags"
            }
            post {
                failure {
                    script {
